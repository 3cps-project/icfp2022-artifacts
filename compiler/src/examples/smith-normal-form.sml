 ...
 *
 * This example is from the MLton benchmarks and was
    ...
    fun tabulate (0, _) = array0()
      | tabulate (n, f) = if (n < 0)
          then raise Size
            ...
            val arr = array_alloc(n, f 0)
            fun init i = if (i < n)
                  then (array_update(arr, i, f i); init(i+1))
                  else ()
            in
              init 1;
              arr
...
fun smaller (a: IntInf.int, b: IntInf.int): bool =
   (a <> 0) andalso (b = 0 orelse (IntInf.abs a < IntInf.abs b))
...
(* should "div" be "quot"? *)
            ...
            and hitPosAgain () = (swapRowLoop (0, pos, pos, pos);
                                  rowLoop (pos + 1))
         in hitPosAgain ()
         end
      fun loop pos =
...
structure Main =
   struct
      fun snf() =
         let val dim = 3
            val big = Matrix.make (dim, dim, f)
            val entry = Matrix.fetch(smithNormalForm big, 2 (*dim - 1*), 2 (*dim - 1*))
         in if entry = 12
               then ()
            else raise Fail "bug"
         end
