 ...
 * All rights reserved.
 *
 * Knuth-Bendix benchmark from SML/NJ.  The original benchmark was written
 * in Caml.
 *)
...
fun try_find f =
  let fun try_find_rec  []    = failwith "try_find"
        | try_find_rec (a::L) = (f a) handle Fail _ => try_find_rec L
  in try_find_rec
  end
...
fun sameTerm (Var n, Var m) = (n = m)
  | sameTerm (Term(f, args1), Term(g, args2)) = let
      fun sameTerms ([], []) = true
        | sameTerms (t1::ts1, t2::ts2) = sameTerm(t1, t2) andalso sameTerms(ts1, ts2)
        | sameTerms _ = false
      in
        (f = g) andalso sameTerms(args1, args2)
      end
  | sameTerm _ = false

fun union (L1 : int list) = let
      fun mem (a : int) =
        let fun mem_rec [] = false
              | mem_rec (b::L) = (a=b) orelse mem_rec L
        in mem_rec
...
fun substitute subst =
  let fun subst_rec (Term(oper,sons)) = Term(oper, List.map subst_rec sons)
        | subst_rec (t as (Var n))     = (assoc n subst) handle Fail _ => t
  in subst_rec
  end
...
fun matching term1 term2 =
  let fun match_rec subst (Var v, M) =
          if mem_assoc v subst then
            if sameTerm(M, assoc v subst) then subst else failwith "matching"
          else
            (v,M) :: subst
...
fun compsubst subst1 subst2 =
  List.map (fn (v,t) => (v, substitute subst1 t)) subst2 @ subst1
      ...
      if mem oper INFIXES then
       ...
       (print_string oper;
        case sons of
          ...
          | t::lt =>(print_string "(";
                     pretty_term t;
                     List.app (fn t => (print_string ","; pretty_term t)) lt;
                     print_string ")"))
...
fun pretty_rules l = List.app pretty_rule l
...
fun mrewrite1 rules =
  let fun rewrec M =
    (mreduce rules M) handle Fail _ =>
      let fun tryrec [] = failwith "mrewrite1"
            | tryrec (son::rest) =
                (rewrec son :: rest) handle Fail _ => son :: tryrec rest
      in case M of
...
fun mrewrite_all rules M =
  let fun rew_loop M =
    rew_loop(mrewrite1 rules M)  handle  Fail _ => M
  in rew_loop M
  end
...
fun diff_eq equiv (x,y) =
  let fun diffrec (p as ([],_)) = p
        | diffrec ((h::t), y) =
            diffrec (t,rem_eq equiv h y) handle Fail _ =>
              let val (x',y') = diffrec (t,y) in (h::x',y') end
  in if List.length x > List.length y then diffrec(y,x) else diffrec(x,y)
  end
...
fun super M =
  let fun suprec (N as Term(_,sons)) =
      let fun collate (pairs,n) son =
                (pairs @ List.map (fn (u,sigma) => (n::u,sigma)) (suprec son), n+1);
          val insides =
                fst (it_list collate ([],1) sons)
      in ([], unify(M,N)) :: insides  handle Fail _ => insides
      end
    | suprec _ = []
  in suprec end
...
fun super_strict M (Term(_,sons)) =
        let fun collate (pairs,n) son =
          (pairs @ List.map (fn (u,sigma) => (n::u,sigma)) (super M son), n+1)
...
fun rename n (t1,t2) =
  let fun ren_rec (Var k) = Var(k+n)
        | ren_rec (Term(oper,sons)) = Term(oper, List.map ren_rec sons)
              ...
              if l<n then next_criticals (1,l+1) else
               (case failures of
                ...
                | _  => (message "Non-orientable equations :";
                         List.app non_orientable failures;
                  ...
                  fun enter_rule(left,right) =
                    let val new_rule = (n+1, mk_rule left right) in
                     (pretty_rule new_rule;
                      ...
                      in (List.app deletion_message redl;
                          let fun right_reduce (m,(_,(L,R))) =
                              (m,mk_rule L (mrewrite_all (new_rule::rules) R));
                              val irreds = List.map right_reduce irredl;
                              val eqs' = List.map (fn (_,(_,pair)) => pair) redl
            ...
            (let val (v,el) = get_rule l in
               if k=l then
                ...
                (let val (_,ek) = get_rule k in
                 ...
                 handle Fail "find" (*rule k deleted*) =>
                   next_criticals (k+1,l))
             end
             handle Fail "find" (*rule l deleted*) =>
...
fun kb_complete greater complete_rules rules =
    let val n = check_rules complete_rules;
        val eqs = List.map (fn (_,(_,pair)) => pair) rules;
        val completed_rules =
